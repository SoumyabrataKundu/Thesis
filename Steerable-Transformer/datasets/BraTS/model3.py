...
class Model(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.convolution_stem2 = torch.nn.Sequential(
            ...
            snn.SE3Conv(channel(4), transformer_dim, 5, n_angle=n_angle, padding='same'),
            snn.SE3BatchNorm(),
        )
        ...
        self.encoder_decoder = torch.nn.Sequential(
            snn.SE3TransformerEncoder(transformer_dim, 4, n_layers=4, add_pos_enc=True),
        )
        
        self.convolution_head1 = torch.nn.Sequential(
            snn.SE3Conv(mult(transformer_dim, 2), channel(4), 5, n_angle=n_angle, padding = 'same'),
            snn.SE3NormNonLinearity(channel(4)),
